...
#define EVENTSIZE 5 //Only a 1-dimensional array for this test
  ...
  switch(Serial.parseInt()) {
    case 1: //Adding values to queue
      for (int i=0; i<EVENTSIZE; i++) {
        ...
        stagingArray[i] = (float) Serial.parseInt();
      }
      pushToQueue();
      break;
    case 2: //Counting values in queue
      ...
      Serial.println(" arrays in the queue.");
      break;
    case 3: //Read one array
      if (writePoint != readPoint) {
        Serial.print("Reading array: ");
        readFromQueue();
        for (int i=0; i<EVENTSIZE; i++) {
          Serial.println(stagingArray[i]);
        }
      }
      else {
      ...
      break;
    case 4: //Pull one array
      if (writePoint != readPoint) {
        Serial.print("Pulling array: ");
        pullFromQueue();
        for (int i=0; i<EVENTSIZE; i++) {
          Serial.println(stagingArray[i]);
        }
